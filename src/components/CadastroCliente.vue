<template>
    ...
    <section class="mainsection">
      <div class="top-section">
        ...
        <div class="modal" v-if="isModalOpen && !clienteEmEdicao">
          <div class="modal-content">
            ...
            <p class="subtitle">Forneça as informações do seu cliente aqui.</p>

            <div class="body-modal">
              <input placeholder="Nome" type="text" v-model="novoCliente.nome">
              <input placeholder="Cidade" type="text" v-model="novoCliente.cidade">
              <button @click="criarCliente">Criar</button>
            </div>
          </div>
        </div>

        <div class="modal" v-if="clienteEmEdicao">
          <div class="modal-content">
            <div class="top-modal">
              <div class="types">
            ...
            <p class="subtitle">Edite as informações do seu cliente aqui.</p>

            <div class="body-modal">
              <input placeholder="Nome" type="text" v-model="clienteEmEdicao.nome">
              <input placeholder="Cidade" type="text" v-model="clienteEmEdicao.cidade">
        ...
        <div class="modal" v-if="confirmarExclusao">
          <div class="modal-content">
            ...
            <p class="subtitle">Tem certeza que quer excluir "{{ clienteParaExclusao.nome }}"?</p>
            <div class="body-modal">
      ...
      <div class="bodycontainer">
        <div class="cardscontainer">
          <div class="card" v-for="(cliente, index) in clientes" :key="index">
            ...
            <div class="topcards">
              ...
              <div class="iconscards">
                ...
                <a @click="deletarCliente(cliente)">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="cidade">
              ...
              <p><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-building-skyscraper" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                <path d="M3 21l18 0" />
...
export default {
  ...
  methods: {
    ...
    criarCliente() {
      if (this.novoCliente.nome && this.novoCliente.cidade) {
        const auth = getAuth();
        if (auth.currentUser) {
          const clientName = this.novoCliente.nome;
          const clientCity = this.novoCliente.cidade;
          this.novoCliente = {
            nome: '',
            cidade: '',
          };
          ...
          const novoClienteKey = novoClienteRef.key;
          set(novoClienteRef, { nome: clientName, cidade: clientCity, id: novoClienteKey });
          this.mostrarAlerta(clientName, 'foi criado com sucesso!');
        } else {
          this.mostrarErrorMessage(
            'Você precisa estar autenticado para criar um cliente.'
          );
        }
      } else {
        this.mostrarValidationError(
          'Por favor, preencha o campo de nome e cidade do cliente.'
        );
      }
    },
    
    editarCliente(cliente) {
      this.clienteEmEdicao = JSON.parse(JSON.stringify(cliente));
      this.isModalOpen = true;
    },
    salvarEdicao() {
      if (this.clienteEmEdicao) {
        const db = getDatabase();
        const clientesRef = ref(db, 'clientes/' + this.clienteEmEdicao.id);
        set(clientesRef, { nome: this.clienteEmEdicao.nome, cidade: this.clienteEmEdicao.cidade, id: this.clienteEmEdicao.id })
          .then(() => {
            this.mostrarAlerta(this.clienteEmEdicao.nome, 'foi editado com sucesso!');
            this.isModalOpen = false;
            this.clienteEmEdicao = null;
            this.listarClientes();
          })
        ...
        console.error('Cliente em edição não encontrado.');
      }
    },
    cancelarEdicao() {
      this.isModalOpen = false;
    ...
    mostrarAlerta(clientName, message) {
      this.alertMessage = `Cliente ${clientName} ${message}`;
      this.showAlert = true;
    ...
    deletarCliente(cliente) {
      if (cliente && cliente.id) {
        this.clienteParaExclusao = cliente;
        this.confirmarExclusao = true;
      }
    },
    cancelarExclusao() {
      this.confirmarExclusao = false;
      this.clienteParaExclusao = null;
    },
    executarExclusao() {
      if (this.clienteParaExclusao && this.clienteParaExclusao.id) {
        const db = getDatabase();
        const clientesRef = ref(db, 'clientes/' + this.clienteParaExclusao.id);
        remove(clientesRef)
          .then(() => {
            this.mostrarAlerta(this.clienteParaExclusao.nome, 'foi excluído com sucesso!');
            this.listarClientes();
        ...
        console.error('Cliente inválido ou sem ID.');
      }
    },
  },
  created() {
